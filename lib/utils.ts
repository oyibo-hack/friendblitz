...
/**
 * Converts a data bundle identifier to its corresponding size (e.g., "1GB", "500MB") based on the mobile network.
 * @param network - The mobile network ("mtn", "airtel", "glo").
...
export function convertBundle(
  ...
  // Define mappings for each network
  const networkBundleMappings: Record<string, Record<string, string>> = {
    mtn: {
      "500": "500MB",
      M1024: "1GB",
    },
  ...
  // Retrieve the bundle mappings for the specified network
  const bundleMappings = networkBundleMappings[network];
  if (!bundleMappings) {
